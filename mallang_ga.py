...
GENE_MUTATION_RATE = 0.2    # mutation을 겪는 개체에서, 각각의 유전자가 mutate 될 확률
GENE_MUTATION_STRENGTH = 0.03   # mutation 되는 유전자에서, gaussian distribution 으로 값을 +- 하는데, 그 distribution의 mean 값

INPUT_AUDIO = "./datasets/10s_datasets/clean/audio_Ballade_4.wav"   # 원본 오디오
TARGET_AUDIO = "./datasets/10s_datasets/Dynamics/processed_audio_6.wav"    # 변조된 오디오 (목표)

RESULT_FOLDER = "experiment_5"  # 각 세대별 최선을 모을 폴더
TEMP_FOLDER = "experiment_5/temp"
...
class Individual:   # 유전자 (float array) 랑 fitness (float), 이름
    ...
    def evaluate(self):
        ...
        sf.write(TEMP_FOLDER + "/" + str(self.i_name) + "_temp_audio.wav", processed_audio, sr)   # 변조된 파일 작성
        fq_fitness, _, _, _ = fitness.fitness_frequency_domain(TEMP_FOLDER + "/" + str(self.i_name) + "_temp_audio.wav", TARGET_AUDIO)    # frequency domain 유사도
        tm_fitness, _, _, _ = fitness.fitness_time_domain(TEMP_FOLDER + "/" + str(self.i_name) + "_temp_audio.wav", TARGET_AUDIO) # time domain 유사도
        self.fitness = 0.5 * fq_fitness + 0.5 - 5 * (1 - tm_fitness)    # 유사도 합치기 (비율 조정)
        os.remove(TEMP_FOLDER + "/" + str(self.i_name) + "_temp_audio.wav")   # 변조된 파일 삭제
